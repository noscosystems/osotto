<?php
    ...
    class ActiveForm extends \CActiveForm
    {
        ...
        public function input(\CForm $form, $attribute, $htmlOptions = array())
        {
            ...
            if(isset($element::$coreTypes[$element->type])) {
                ...
                if($element->type == 'date' || $method == 'activeDateField') {
                    $this->convertTimestamp($form->model, $element->name);
                }
                // Render element.
         ...
         * @return void
         */
        public function convertTimestamp(\CModel $model, $attribute)
        {
            if(isset($model->$attribute) && preg_match('/^\\-?[1-9][0-9]*$/', $model->$attribute)) {
